---
title: "SG ethnicity categorical"
author: "Wu Yiyang"
date: "2024-02-27"
output: pdf_document
---
...
my.dat$Ethnicity <- factor(my.dat$Ethnicity, 
                           ...
                           labels = c("White", "His/Lat", "Black", "Mixed", "Asian", "Others", "Native", "Chinese", "Malay", "Indian", "Eurasian"))
...
con_fin<-confint(modelFinLevel3_new,level = 0.95)
con_fin
...
```{r}
library(knitr)
model_summary <- summary(modelFinLevel3)
...
kable(coefficients_df, digits = 3, caption = "SG financial model with categorical Ethnicity (without AIC)")
```
\newpage
# SG financial model with categorical Ethnicity

```{r}
model_summary <- summary(modelFinLevel3_new)
coefficients_df <- data.frame(
  Estimate = model_summary$coefficients[, "Estimate"],
  StdError = model_summary$coefficients[, "Std. Error"],
  Zvalue = model_summary$coefficients[, "z value"],
...
kable(coefficients_df, digits = 3, caption = "SG financial model with categorical Ethnicity")
...
kable(results_df, digits = 3, caption = "95% CIs")
```

Baseline of Religion is Buddhism.

McFadden pseudo - $R^2$ to measure how well the model fits:
$pR^2_{MF} = 1-(df_{residual}/df_{null})$ = $1-294.59/413.82=0.288$
...
con_fin<-confint(modelMedLevel3_new,level = 0.95)
con_fin
...
model_summary <- summary(modelMedLevel3)
coefficients_df <- data.frame(
  Estimate = model_summary$coefficients[, "Estimate"],
  StdError = model_summary$coefficients[, "Std. Error"],
  Zvalue = model_summary$coefficients[, "z value"],
...
kable(coefficients_df, digits = 3, caption = "SG Medical model with categorical Ethnicity (without AIC)")
```

\newpage
# SG medical model with categorical Ethnicity

```{r}
model_summary <- summary(modelMedLevel3_new)
coefficients_df <- data.frame(
  Estimate = model_summary$coefficients[, "Estimate"],
  StdError = model_summary$coefficients[, "Std. Error"],
  Zvalue = model_summary$coefficients[, "z value"],
...
kable(coefficients_df, digits = 3, caption = "SG Medical model with categorical Ethnicity")
...
Baseline of Ethnicity is Chinese.

McFadden pseudo - $R^2$ to measure how well the model fits:
$pR^2_{MF} = 1-(df_{residual}/df_{null})$ = $1-311.76/412.49=0.244$
